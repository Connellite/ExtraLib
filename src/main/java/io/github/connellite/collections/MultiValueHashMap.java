...
public class MultiValueHashMap<K, V> extends HashMap<K, List<V>>
    ...
    /**
     * Create a new {@code MultiValueHashMap} that wraps a {@link HashMap}.
     */
     ...
     */
    public MultiValueHashMap(int expectedSize) {
        super(expectedSize);
    }

    /**
     * Copy constructor: Create a new {@code MultiValueHashMap} with the same mappings as
     * the specified Map.
     *
     ...
     * @param otherMap the Map whose mappings are to be placed in this Map
     */
    public MultiValueHashMap(MultiValueMap<K, V> otherMap) {
        super(otherMap);
    }
    ...
    public Map<K, List<V>> toUnmodifiableMap() {
        Map<K, List<V>> immutableMap = new HashMap<>(size());
     ...
     */
    public MultiValueHashMap<K, V> deepCopy() {
        MultiValueHashMap<K, V> copy = new MultiValueHashMap<>(size());
    ...
    @Override
    public MultiValueHashMap<K, V> clone() {
